...
provider "aws" {
  ...
  # TODO: Update this to use 3.22, otherwise CloudTrail will not deploy.
  # We are using a `source` attribute on the provider, which is not
  # supported in Terraform 0.12, so we also need to update this example to use Terraform 0.13 or later.
  version = "~> 2.6"
...
----
terraform {
  # TODO: Wow, this syntax is horribly wrong.. definitely trips you up. I only noticed it because I work here.
...
----
module "security_baseline" {
# TODO: we should update this so that we support terraform 13/14!
